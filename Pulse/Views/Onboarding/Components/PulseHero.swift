// PulseHero.swift
// Animated hero for the Welcome screen:
//   - radial coral wash background
//   - three expanding ring pulses (staggered, looping)
//   - central heart that scales with a heartbeat rhythm
//   - ECG line drawn/undrawn across the middle
//
// Pure SwiftUI, no external dependencies. Uses TimelineView + trimmed shapes.
...
struct PulseHero: View {
    ...
    private func ringPhase(t: TimeInterval, offset: Double) -> CGFloat {
        ...
        let p = ((t + offset).truncatingRemainder(dividingBy: period)) / period
        return CGFloat(p)
    }

    private func heartScale(t: TimeInterval) -> CGFloat {
        // Double-thump heartbeat, ~60 bpm
    ...
    private func ecgTrim(t: TimeInterval) -> CGFloat {
        ...
        // draw 0-0.6, hold 0.6-0.75, undraw 0.75-1.0
